...
final class MonocleWindow extends Window {
    ...
    /**
     * w/h is the total window width/height including all its decorations (e.g. title bar). cw/ch is
     * the "client", or interior width/height. Negative values for w/h/cw/ch are treated as "not
     * set". For example: setBounds(x, y, xSet, ySet, 800, 600, -1, -1) will make the window 800x600
     * pixels. The client area available for drawing will be smaller, e.g. 792x580 - it depends on
     * the window decorations on different platforms. setBounds(x, y, xSet, ySet, -1, -1, 800, 600)
     * will make the window client size to be 800x600 pixels. The area for drawing (FX scene size)
     * will be exactly 800x600, but the total window size including decorations will be slightly
     * bigger. For undecorated windows w/h and cw/ch are obviously the same. As this is a void
     * function the native code should trigger an event to notify the system on actual change
     */
    @Override
    ...
    protected void _setEnabled(long ptr, boolean enabled) {
        if (!enabled && (this == MonocleWindowManager.getInstance().getFocusedWindow())) {
            MonocleView view = (MonocleView) getView();
            try {
                view.notifyMouse(MouseEvent.EXIT, MouseEvent.BUTTON_NONE, 0, 0, 0, 0, 0, false, false);
    ...
    /**
     * The functions below are used when the platform support modality natively. Currently only GTK
     * is using it. This functionality is disabled by default. In order to enable it this class need
     * to override Window:: supportsPlatformModality() to return true.
     */
    @Override
