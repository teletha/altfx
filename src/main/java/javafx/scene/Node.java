...
public abstract class Node implements EventTarget, Styleable {
                ...
                protected void invalidated() {
                    if (oldParent != null) {
                        if (nodeTransformation != null && nodeTransformation.listenerReasons > 0) {
                            oldParent.localToSceneTransformProperty()
                    ...
                    if (newParent != null) {
                        if (nodeTransformation != null && nodeTransformation.listenerReasons > 0) {
                            newParent.localToSceneTransformProperty().addListener(nodeTransformation.getLocalToSceneInvalidationListener());
                        }
                        //
    ...
    private static class TransitionTimerCollection extends HashMap<String, TransitionTimer> {
         ...
         *
         * @param property the {@code StyleableProperty}
         * @param result this argument is an implementation detail, callers must specify
         *            {@code null}
         * @return a mapping of CSS property names to {@code TransitionTimer} instances
         */
        private Map<String, TransitionTimer> collectTransitionTimers(StyleableProperty<?> property, Map<String, TransitionTimer> result) {
     ...
     */
    private static class TransitionDefinitionCollection extends ArrayList<TransitionDefinition>
         ...
         */
        public Map<CssMetaData<? extends Styleable, ?>, TransitionDefinition> findAll(CssMetaData<? extends Styleable, ?> metadata) {
            if (isEmpty()) {
                return Map.of();
            }
        ...
        /**
         * Finds all transition definitions for the property referenced by the specified CSS
         * metadata,
         * including all of its sub-properties. This method does not allocate a new {@code Map} for
         * the
         ...
         * @return a mapping of CSS metadata instances to {@code TransitionDefinition} instances
         */
        private Map<CssMetaData<? extends Styleable, ?>, TransitionDefinition> collectTransitions(CssMetaData<? extends Styleable, ?> metadata, Map<CssMetaData<? extends Styleable, ?>, TransitionDefinition> result) {
            // We look for a matching transition in reverse, since multiple transitions might be
            // specified
            // for the same property. In this case, the last transition takes precedence.
            ...
            // We also need to search for transitions defined on all sub-properties of the current
            // property.
            for (int i = 0, max = subMetadata.size(); i < max; ++i) {
    ...
    private static abstract class LazyTransformProperty extends ReadOnlyObjectPropertyBase<Transform> {
    ...
    private static abstract class LazyBoundsProperty extends ReadOnlyObjectPropertyBase<Bounds> {
        private boolean valid;
